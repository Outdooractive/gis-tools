...
struct GeoPackageRelatedTablesTests {
    ...
    // Validates writing and reading an attribute table.
    @Test
    func attributeTableRoundTrip() async throws {
        try FeatureCollection([Feature(Point(Coordinate3D(latitude: 10.0, longitude: 20.0)))]).writeGeopackage(to: testUrl())
        ...
        let db = try SQLiteDB(path: testUrl().path)
        defer { db.close() }
        let sqlRows = try db.query("SELECT rowid, * FROM \"station_info\";")
        #expect(sqlRows.count == 2, "Expected 2 rows; got \(sqlRows.count)")
    ...
    // Validates writing and reading a media table.
    @Test
    ...
    // Validates writing and reading relationships.
    @Test
    ...
    // Validates that an attribute table passes validation.
    @Test
    func attributeTableValidation() async throws {
        // Write features to create valid GPKG metadata
        try FeatureCollection([Feature(Point(Coordinate3D(latitude: 0.0, longitude: 0.0)))]).writeGeopackage(to: testUrl())
    ...
    // Validates Feature.loadRelatedAttributes resolves relationship.
    @Test
    func loadRelatedAttributesViaFeatureCollection() async throws {
        ...
        let attrs = try FeatureCollection.loadRelatedAttributes(
            for: "features", from: testUrl())
        #expect(attrs != nil)
        #expect(attrs?.tableName == "info")
        #expect(attrs?.rows.count == 1)
    }

    // Validates Feature.relatedAttributes reads data for a specific feature.
    @Test
    func featureRelatedAttributes() async throws {
        ...
        defer { db.close() }
        try GeoPackage.registerRelatedTablesExtension(in: db)
        let rel = RelationRow(id: "r1", tableName: "features", relatedTableName: "info", relationName: .attributes)
        ...
        // The single feature should have id = .int(1)
        guard let feature = fc.features.first, case .int(let rowId) = feature.id else { return }
        #expect(rowId == 1)
