...
extension Coordinate3D {
    ...
    public init?(dms: String) {
        ...
        let lat = (dir1 == "N" ? 1.0 : -1.0) * (d1 + m1 / 60.0 + s1 / 3600.0)
        let lon = (dir2 == "E" ? 1.0 : -1.0) * (d2 + m2 / 60.0 + s2 / 3600.0)
    ...
    public init(
        ...
        let lat = (latitudeDirection.uppercased() == "N" ? 1.0 : -1.0)
            * (latitudeDegrees + latitudeMinutes / 60.0 + latitudeSeconds / 3600.0)
        let lon = (longitudeDirection.uppercased() == "E" ? 1.0 : -1.0)
...
extension Coordinate3D {
    ...
    public init?(
        ...
        self.init(latitude: lat * 180.0 / .pi, longitude: lon * 180.0 / .pi)
    }
